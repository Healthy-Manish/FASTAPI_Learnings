...
class Patient(BaseModel):
    id: Annotated[str,Field(...,description="ID of patient",examples=['POO1'])]
...
def sort_patient(sort_by: str=Query(...,description="sort on the basis of height , weight or bmi"),
    ...
    if order not in ['asc','desc']:
        raise HTTPException(status_code=400, detail =f'Invalid field select from {order}')
...
def create_patient(patient: Patient):
    ...
    #check if new patient's id exists already
